...
import { SectionFrame } from "@/components/home-v3/SectionFrame";
...
const solutions: Solution[] = [
  {
    ...
    title: "Operational Excellence â€” our digital twin",
...
function SolutionCarousel() {
  ...
  return (
    ...
    >
      {/* Crossfaded cards â€” all stacked, only the active one is opaque. The
        ...
        {solutions.map((item, i) => {
          ...
          return (
            <div
              key={item.title}
              aria-hidden={!on}
              className={`group col-start-1 row-start-1 overflow-hidden rounded-[2rem] border border-white/10 bg-[#0c1330] shadow-2xl transition-all duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] ${
                on
                  ? "opacity-100 blur-0 translate-y-0"
                  : "pointer-events-none translate-y-3 opacity-0 blur-sm"
              }`}
            >
              {/* Top strip â€” ambient atmosphere image */}
      ...
      {/* Navigation â€” dots on the left, arrows on the right */}
...
export function Solutions() {
  return (
    <SectionFrame
      id="solutions"
      outerClassName="bg-night-deep bg-dots-light"
      cardClassName="bg-night-deep"
    >
